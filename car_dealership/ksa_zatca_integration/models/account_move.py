...
class AccountMove(models.Model):
    ...
    def create_xml_file(self, previous_hash=0):
        ...
        if is_tax_invoice:
            if conf.csr_invoice_type not in ['1100', '1000']:
            ...
            if not self.l10n_is_exports_invoice:
                ...
                if not self.partner_id.additional_no or not self.partner_id.state_id.id:
                    message = 'Some required Values are missing in Customer Address, ' \
                              'which are required for tax invoices, in case of non-ksa resident.'
            if not self.partner_id.street or not self.partner_id.building_no or\
                    not self.partner_id.district or not self.partner_id.city or \
                    not self.partner_id.zip or not self.partner_id.country_id.id:
                message = 'Some Values are missing in Customer Address, which are required for tax invoices'
                raise exceptions.ValidationError(message)
        if not is_tax_invoice and conf.csr_invoice_type not in ['1100', '0100']:
            raise exceptions.AccessDenied("Certificate not allowed for Standard Invoices.")
